package com.example.berzanbayram

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.MotionEvent
import android.view.View
import android.view.ViewGroup
import android.widget.*

// TODO: Rename parameter arguments, choose names that match
// the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
private const val ARG_PARAM1 = "param1"
private const val ARG_PARAM2 = "param2"

/**
 * A simple [Fragment] subclass.
 * Use the [f_visibility.newInstance] factory method to
 * create an instance of this fragment.
 */
class f_visibility : Fragment() {
    // TODO: Rename and change types of parameters
    private var param1: String? = null
    private var param2: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        arguments?.let {
            param1 = it.getString(ARG_PARAM1)
            param2 = it.getString(ARG_PARAM2)
        }
    }

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        val view: View = inflater!!.inflate(R.layout.fragment_f_visibility, container, false)
        val buton1 = view.findViewById<Button>(R.id.buton1)
        val buton2 = view.findViewById<Button>(R.id.buton2)
        val textview1 = view.findViewById<TextView>(R.id.textview1)
        val textview2 = view.findViewById<TextView>(R.id.textview2)
        val radiobutton1 = view.findViewById<RadioButton>(R.id.radiobutton1)
        val radiobutton2 = view.findViewById<RadioButton>(R.id.radiobutton2)
        val checkbox1 = view.findViewById<CheckBox>(R.id.checkbox1)
        val checkbox2 = view.findViewById<CheckBox>(R.id.checkbox2)


        var choose = 0

        buton1.setOnClickListener {
            if (choose == 0) {
                buton2.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                buton2.visibility = View.VISIBLE
                choose = 0
            }

        }

        buton2.setOnClickListener {
            if (choose == 0) {
                buton1.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                buton1.visibility = View.VISIBLE
                choose = 0
            }
        }
        textview1.setOnClickListener {
            if (choose == 0) {
                textview2.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                textview2.visibility = View.VISIBLE
                choose = 0
            }

        }
        textview2.setOnClickListener {
            if (choose == 0) {
                textview1.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                textview1.visibility = View.VISIBLE
                choose = 0
            }
        }
        radiobutton1.setOnClickListener {
            if (choose == 0) {
                radiobutton2.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                radiobutton2.visibility = View.VISIBLE
                choose = 0
            }

        }
        radiobutton2.setOnClickListener {
            if (choose == 0) {
                radiobutton1.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                radiobutton1.visibility = View.VISIBLE
                choose = 0
            }
        }
        checkbox1.setOnClickListener {
            if (choose == 0) {
                checkbox2.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                checkbox2.visibility = View.VISIBLE
                choose = 0
            }

        }
        checkbox2 .setOnClickListener {
            if (choose == 0) {
                checkbox1.visibility = View.INVISIBLE
                choose = 1
            } else if (choose == 1) {
                checkbox1.visibility = View.VISIBLE
                choose = 0
            }
        }

        return view
    }

    companion object {
        /**
         * Use this factory method to create a new instance of
         * this fragment using the provided parameters.
         *
         * @param param1 Parameter 1.
         * @param param2 Parameter 2.
         * @return A new instance of fragment f_visibility.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance(param1: String, param2: String) =
            f_visibility().apply {
                arguments = Bundle().apply {
                    putString(ARG_PARAM1, param1)
                    putString(ARG_PARAM2, param2)
                }
            }
    }
}